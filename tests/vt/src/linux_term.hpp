    ...
    for (;;) { // BEGIN LOOP_OVER_POLL
      ...
      while (current < last) {
        ...
        case parse_state::expecting_control_sequence: {
          ...
          break;
        } // parse_state::expecting_control_sequence
            ...
            *current_param = (*current_param * 10) + (c - '0');
            ...
            case '~': { // extension function key
              for (int i = 0; i < last; ++i) {
                if (isprint(buffer[i])) {
                  std::clog << vt100::red << buffer[i];
                } else {
                  std::clog << vt100::setf(245, 95, 0) << (int)buffer[i];
                }
              }
              std::clog << vt100::reset << '\n';
              assert(current_param <= num_parameters + 1 &&
                     "Unknown numeric sequence for extended function key");
              result = parse_function_key(
                  (char)num_parameters[0], term_events::f4,
      ...
      default:
        copy_leftover_to_beginning(); // We're in the middle of a parse, but
                                      // the result is entirely determined by
                                      // what follows
      }
    } // END LOOP_OVER_POLL
